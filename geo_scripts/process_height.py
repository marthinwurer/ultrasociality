...
def aggregate_grid(raster: gr.GeoRaster, x_ranges, y_ranges, func=np.mean):
    """
    ...
    for y_step, y_range in enumerate(y_ranges):
        for x_step, x_range in enumerate(x_ranges):
            ...
            nw_corner = map_indexes(raster, x_range[0], y_range[0])
            se_corner = map_indexes(raster, x_range[1], y_range[1])
            im_slice = raster.raster[nw_corner[0]:se_corner[0], nw_corner[1]:se_corner[1]]
            val = func(im_slice)
            output[y_step, x_step] = val  # I don't know why these need to be flipped.
...
def br_wrapper(raster, x_deg, y_deg):
    """
